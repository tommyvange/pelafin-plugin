...
The mode is stored as `search.seerr.seriesRequestMode` (`seasons` or `all`) in the shared app configuration. It controls the selection interface and sends the existing explicit season-array request; it does not change permissions or add an endpoint. The preview requires the user to submit the request explicitly. The optional result-card buttons submit directly, as described below. Requests use standard quality and Seerr's configured defaults. Each user's permissions, quotas, and approval rules still apply. The plugin checks availability and selected seasons again before submitting and rejects special-season requests (season 0), including manually crafted requests. Automatic retries are disabled; after a timeout, check Seerr before resending because the request may have arrived.

**Settings → Search → Show request buttons under results** controls the buttons below requestable posters in both layouts. They are shown by default and saved as `search.seerr.showRequestButtons` in the shared configuration. Clicking one sends a request immediately as the signed-in user, without opening or confirming in the preview. For shows, it first reads current availability and requests every eligible regular season, regardless of the preview’s season-selection mode; specials, available seasons, and already-requested seasons are excluded. The button shows progress, prevents repeat clicks while submitting, and displays the result. Automatic retries are disabled. Hiding these buttons leaves the posters and titles clickable for preview-based requests. This uses the existing plugin endpoints and requires no additional plugin update.

**Settings → Search → Request messages** lets administrators add a separate **Movie request message** and **Show request message** for everyone on the server. Use these for approval policies or estimated wait times. Each optional field accepts up to 1,000 characters of plain text, preserves line breaks, and saves when you leave the field; blank messages are hidden. The matching message appears in the movie/show request preview and in the success notification for both preview and direct requests. The text is informational: it does not change approval rules, download queues, or delivery times. Messages default to blank; the example placeholders are not published automatically. They are stored in the shared app configuration as `search.seerr.movieRequestMessage` and `search.seerr.showRequestMessage`, require no new endpoint, and need no plugin update.
